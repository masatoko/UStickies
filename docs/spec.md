...
- プロジェクト設定でカスタム背景色が無効の場合はカテゴリ色、有効の場合はプロジェクト共通色を使用する。
- 背景の不透明度は色とは独立したプロジェクト共通設定とし、初期値は0.35とする。
- 文字サイズはUnity標準値を初期状態とし、プロジェクト設定で8pxから48pxの範囲へ変更できる。
- 枠線、カテゴリ名、Done状態の文字、カテゴリ色の帯、接続線は表示しない。
...
- 内容カードのカスタム背景色の使用有無、背景色、不透明度を設定する。値はプロジェクト全体で共有する。
- 内容カードのカスタム文字サイズの使用有無と文字サイズを設定する。値はプロジェクト全体で共有する。
