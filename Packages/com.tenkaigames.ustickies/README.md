...
- 本文、カテゴリ、Done状態、Pin状態、Scene View上の表示オフセットの編集
...
| -------- | -------------------- | --------------- | ------------ |
| `v0.2.0` | `0.2.0`              | 6000.3          | 対応         |
| `v0.2.0` | `0.2.0`              | 6000.6          | 動作確認済み |

Unity 6000.3および6000.6では `v0.2.0` の利用を推奨する。
...
```text
https://github.com/masatoko/UStickies.git?path=/Packages/com.tenkaigames.ustickies#v0.2.0
```
...
| `Ctrl/Cmd + 左ドラッグ`  | 表示位置を調整可能   | 表示位置を調整可能        |
...
- **表示位置だけを移動**
  Scene View上のアイコンを `Ctrl/Cmd + 左ドラッグ` する。ワールド座標やGameObjectとの紐づけ位置は変化しない。
...
- `Use Custom Font Size` をONにすると、`Font Size` を8pxから48pxの範囲で指定できる。設定はプロジェクト全体で共有される。
